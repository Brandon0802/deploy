...
if($gettrans){
...
}else if($postjson['aksi'] == "get_accs"){


  
  $caccs = mysqli_query($mysqli, "SELECT c.user_id, c.first_name, c.last_name, c.contact_number, c.type, b.barangay_name FROM consumer_account as c, barangay as b WHERE c.barangay_id=b.barangay_id");
  $daccs = mysqli_query($mysqli, "SELECT d.user_id, d.first_name, d.last_name, d.contact_number, b.barangay_name FROM driver_account as d, barangay as b WHERE d.barangay_id=b.barangay_id");


  $data = array();

  while($rows = mysqli_fetch_array($caccs)){
    $data[] = array(
      'user_id'=> $rows['user_id'],
      'first_name' =>$rows['first_name'],
      'last_name' =>$rows['last_name'],
  ...
  while($rows2 = mysqli_fetch_array($daccs)){
    $data2[] = array(
      'user_id'=> $rows2['user_id'],
      'first_name' =>$rows2['first_name'],
      'last_name' =>$rows2['last_name'],
      'contact_number' =>$rows2['contact_number'],
      'barangay' => $rows2['barangay_name'],
      'type' =>'Driver'
...
if($caccs){
  $result = json_encode(array('success' => true, 'result' => $data,'result2' => $data2));
  echo $result;
}
else{
  $result = json_encode(array('success' => false));
